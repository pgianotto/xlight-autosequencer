...
- Python 3.11+ + `mcp` 2.x (`mcp.server.mcpserver.MCPServer` — 1.x's `FastMCP` was renamed and the API changed), `uvicorn[standard]`, `asgiref` (WSGI→ASGI bridge for the existing Flask app). New `src/review/mcp_server.py` exposes the pipeline (library, import, analyze, song story, theme/effect/variant catalogs, generate, layout) as MCP tools over Streamable HTTP, merged into the existing review server's process at `/mcp` (`src/cli/review.py`, port 5173) rather than a second process/port. No new analysis/generation/export logic — every tool wraps an existing `src/review/api/v1/*` function. (mcp-tool-server)
- JSON files (existing library/session state, unchanged). Serving mechanics only: `src/cli/review.py` now launches via `uvicorn.run()` over a merged `starlette.routing.Router` instead of Flask's own dev server — see `openspec/changes/mcp-tool-server/design.html` for why (SSE-under-uvicorn required a specific `asgiref.sync.ThreadSensitiveContext` fix, not just "run uvicorn"; `/mcp` routing required a bare `Router(default=...)` rather than sibling `Mount`s). (mcp-tool-server)
...
## Recent Changes
- mcp-tool-server: Added Python 3.11+ + `mcp` 2.x, `uvicorn[standard]`, `asgiref`. New `src/review/mcp_server.py` exposes the pipeline as MCP tools over Streamable HTTP at `/mcp`, merged into the existing review server process (`src/cli/review.py`). See `openspec/changes/mcp-tool-server/`.
