#!/usr/bin/env python3
"""Mine existing xLights sequences for arch effect/timing/audio patterns.

Read-only against every .xsq/.xsqz file — nothing under the scanned show
folder is ever written to. Walks a show folder, finds every sequence, and
for each one:

  - Parses the sequence XML for effect placements on arch models and arch
    model groups (never modifies the file).
  - Parses xlights_rgbeffects.xml for arch models and modelGroups so
    "Arches" (a group) and "Arch1"/"Arch-L" (individual models) are both
    captured.
  ...
  - Runs a librosa-only beat/energy pass on that audio (no vamp/madmom
    dependency) and correlates arch effect boundaries against beats and
    energy level.
...
Usage:
    python scripts/mine_arch_corpus.py "E:\\2023\\ShowFolder3D" \\
...
# Elements to exclude from arch mining even though their name/membership
# matches the arch token — user-requested exclusion: "Triple Arches" is a
# separate prop family from the "Arches" family and shouldn't be blended
# into the same corpus.
_EXCLUDED_ELEMENT_TOKENS = ("triple arch",)
...
class SongMining:
    ...
    arch_elements: list[ArchElement] = field(default_factory=list)
...
# ---------------------------------------------------------------------------
# Layout parsing (arch models + arch model groups)
# ---------------------------------------------------------------------------
...
def parse_arch_layout(layout_path: Path) -> dict[str, ArchElement]:
    """Return arch elements keyed by name: individual arch models, plus any
    modelGroup whose own name mentions "arch" or whose members include an
    arch model. Elements matching _EXCLUDED_ELEMENT_TOKENS (e.g. "Triple
    Arches", a separate prop family) are dropped entirely."""
    tree = ET.parse(layout_path)
    root = tree.getroot()

    arch_models: set[str] = set()
    for model_el in root.findall(".//model"):
        name = model_el.get("name", "")
        if not name:
            continue
        if _ARCH_TOKEN in name.lower() and not _is_excluded_element_name(name):
            arch_models.add(name)

    elements: dict[str, ArchElement] = {
        name: ArchElement(name=name, kind="model") for name in arch_models
    }
    ...
    for group_el in root.findall(".//modelGroup"):
        ...
        name_matches = _ARCH_TOKEN in group_name.lower()
        # Membership alone only counts a group as "arch" when arches are the
        # majority of its members — otherwise whole-house/whole-yard umbrella
        # groups that merely include an arch among 100+ other props would be
        # misclassified as arch groups and swamp the corpus with noise.
        arch_member_count = sum(1 for m in members if _ARCH_TOKEN in m.lower())
        majority_arch_members = bool(members) and arch_member_count / len(members) > 0.5
        if group_name and (name_matches or majority_arch_members):
            elements[group_name] = ArchElement(
...
def mine_sequence(xsq_path: Path, arch_elements: dict[str, ArchElement]) -> SongMining:
    ...
    if ee is not None:
        for elem_el in ee.findall("Element"):
            ...
            if name not in arch_elements:
                continue
            kind = arch_elements[name].kind
    ...
    song.arch_elements = [
        el for name, el in arch_elements.items()
...
def write_song_summary_md(song: SongMining, out_dir: Path) -> None:
    lines = []
    lines.append(f"# Arch sequencing summary: {song.xsq_path.name}")
    lines.append("")
    ...
    lines.append("## Arch elements found")
    for el in song.arch_elements:
        if el.kind == "group":
    ...
    if white_base or color_mask:
        lines.append("## White-base / color-mask layering technique (on arch groups)")
        lines.append(
...
def rebuild_index_md(out_root: Path) -> int:
    ...
    lines = ["# Arch sequencing corpus — index", ""]
    ...
    for slug, s in songs:
        ...
        lines.append(
            f"- [{xsq_name}]({slug}/summary.md) — {s.get('placement_count', 0)} arch placements, "
...
def main() -> None:
    ...
    parser.add_argument("--out", type=Path, default=Path("docs/arch_sequencing_corpus"))
    ...
    arch_elements: dict[str, ArchElement] = {}
    if layout_path is not None:
        arch_elements = parse_arch_layout(layout_path)
        print(f"Layout: {layout_path}")
        print(f"Arch elements in layout: {len(arch_elements)} -> {sorted(arch_elements)}")
    else:
    ...
    extract_root = Path(tempfile.mkdtemp(prefix="arch_corpus_pkg_"))
    ...
    for xsq_path in xsq_paths:
        ...
        if xsq_path.suffix.lower() in (".xsqz", ".zip"):
            ...
            if package_dir is not None:
                ...
                elements = parse_arch_layout(pkg_layout)
                ...
                print(f"  {xsq_path.name}: self-contained package — "
                      f"embedded layout has {len(elements)} arch element(s)")

        if mine_path is xsq_path and not elements:
            print(f"  SKIP {xsq_path.name}: no layout to resolve arch elements against")
            continue
        ...
        if not song.placements:
            print(f"  {xsq_path.name}: no arch effect placements, skipping")
            continue
    ...
    indexed = rebuild_index_md(args.out)
    print(f"\nMined {len(mined)} song(s) this run; INDEX.md now covers {indexed} song(s) in {args.out}")
